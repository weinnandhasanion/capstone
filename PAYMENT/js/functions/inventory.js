...
renderCard = () => {
    ...
    if(status.length > 0) {
        ...
        tr.innerHTML = 
         ...
         <a class='btn-link' onclick='viewInventory(this.parentNode.parentNode)'
          ...
          <i data-toggle="tooltip" data-placement="top" title="view ${inv.data.name}"
  ...
  searchInv.onkeyup = () => {
     ...
     // Searching deleted Inventory
  let searchDel = document.getElementById('search-delete');

  searchDel.onkeyup = () => {
   
      if(searchDel.value != '' ) {
        let docs = filtering(searchDel.value);
        deleteTable(docs);
      } else {
        renderDelete();
      }
    ...
    deleteTable = (docs) => {
      let InventoryCont = document.getElementById('tbody');
      ...
      docs.forEach((doc) => {
        ...
        tr.innerHTML = 
         ...
         style="font-size:20px; cursor:pointer;" class="fas fa-eye"></i>  
           ...
           <i id="deleteBtn" onclick='recoverInv(this.parentNode.parentNode)'
               data-toggle="tooltip" data-placement="top" title="Recover ${doc.data.name}"
               class="fas fa-undo-alt mx-4" style="cursor:pointer; font-size: 20px; color: #32CD32" ></i>
              
         </td>`;
  
       
          InventoryCont.appendChild(tr);
        });
      } else {
